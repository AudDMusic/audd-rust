...
Every typed model carries an `extras: HashMap<String, serde_json::Value>` populated via `#[serde(flatten)]`. It's the supported way to read fields outside the typed surface, including provider blocks the typed shape doesn't expose:
