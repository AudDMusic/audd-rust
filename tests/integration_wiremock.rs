...
async fn streams_longpoll_preflight_no_callback_raises() {
    ...
    let res = audd
        ...
        .await;
    let e = res.err().expect("should have failed");
    assert!(e.is_invalid_request(), "got {e:?}");
}

#[tokio::test]
async fn streams_longpoll_skip_check_runs() {
    let server = MockServer::start().await;
    ...
    assert!(first.get("timeout").is_some());
}
...
#[tokio::test]
async fn tokenless_longpoll_returns_event() {
    let server = MockServer::start().await;
    Mock::given(method("GET"))
        .and(path("/longpoll/"))
        .respond_with(ResponseTemplate::new(200).set_body_json(json!({
    ...
    let consumer = LongpollConsumer::builder("abc")
        ...
        .build()
        .unwrap();
    let mut s = consumer.iterate(LongpollIterateOptions {
        timeout: 1,
        ..Default::default()
    });
    let first = s.next().await.unwrap().unwrap();
    assert_eq!(first.get("timestamp").and_then(|v| v.as_i64()), Some(42));
}
