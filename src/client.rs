...
impl AudD {
    ...
    pub async fn recognize(
        ...
        self.recognize_with(source, RecognizeOptions::default()).await
    }
    ...
    pub async fn recognize_with(
        ...
        let return_str = opts.return_metadata.map(|v| v.join(","));
        let market = opts.market.map(str::to_string);
        let timeout = opts.timeout;
        let extra_parameters: Option<Vec<(String, String)>> = opts
            .extra_parameters
        ...
        let resp = match retry_async(
            || {
                ...
                async move {
                    ...
                    if let Some(extras) = extra_parameters.as_ref() {
                        for (k, v) in extras {
    ...
    pub async fn recognize_enterprise(
        ...
        let extra: Vec<(&'static str, String)> = build_enterprise_fields(return_str.as_deref(), &opts);
        let extra_parameters: Option<Vec<(String, String)>> = opts
            .extra_parameters
            .map(|m| m.iter().map(|(k, v)| (k.clone(), v.clone())).collect());
        ...
        let resp = match retry_async(
            || {
                ...
                async move {
                    ...
                    let mut fields: Vec<(&str, String)> = Vec::new();
                    // extra_parameters first; typed fields override on collision.
                    if let Some(ex) = extra_parameters.as_ref() {
                        for (k, v) in ex {
                            fields.push((k.as_str(), v.clone()));
                        }
                    }
                    ...
                    http.post_form(&url, &fields, Some(form), opts.timeout).await
                }
            },
            self.inner.recognition_policy(),
        )
...
/// Optional parameters for [`AudD::recognize_with`].
#[derive(Debug, Default)]
pub struct RecognizeOptions<'a> {
    /// Metadata services to fetch on a match (`apple_music`, `spotify`,
    /// `deezer`, `napster`, `musicbrainz`).
    pub return_metadata: Option<&'a [String]>,
    /// ISO country code for Apple Music / Spotify market scoping.
    pub market: Option<&'a str>,
    /// Per-call timeout override.
    pub timeout: Option<Duration>,
    /// Additional form fields the typed options don't cover —
    /// undocumented parameters, beta features. Typed fields win on
    /// collision.
    pub extra_parameters: Option<&'a HashMap<String, String>>,
}
